...
function initDrag() {
    slides.forEach((slide, i) => {
        let moveTarget = slide.querySelector('.col__content');
        let pack = false;
        let touch_p = {
            c_x : 0,
            c_y : 0,
        ...
        function touches(e) {
            switch(e.type) {
                case 'touchstart':
                    ...
                    touch_p.c_x = e.touches[0].clientX;
                    touch_p.c_y = e.touches[0].clientY;
                    break;
                case 'touchend':
                    // moved
                    if (diff_x < 0) {
                        if (-diff_x + 10 < tempX / 2) {
                            // 复位
                            moveTarget.style.transform = `translate(0px)`
                        } else {
                            // 收起
                            moveTarget.style.transform = `translate(-100%)`
                        }
                    }
                    console.log('Touch end(' + diff_x + ')');
                    break;
                case 'touchmove':
                    tempX = e.changedTouches[0].clientX;
                    tempY = e.changedTouches[0].clientY;
                    diff_x = tempX - touch_p.c_x;
                    diff_y = Math.abs(tempY - touch_p.c_y);
                    console.log('Touch move('+e.changedTouches[0].clientX+', '+e.changedTouches[0].clientY+')\ndiff_x=' + diff_x
                    + "\ndiff_y=" + diff_y);

                    // 逻辑实现
                    // moving
                    moveTarget.style.transform = `translate(${diff_x < 0 ? diff_x : 0}px)`

                    break;
            }
        }
        slide.addEventListener('touchstart',touches,false);
        slide.addEventListener('touchend',touches,false);
        slide.addEventListener('touchmove',touches,false);
    });
}
...
function init() {
	...
	initSlides();
	initParallax();
    if (window.innerWidth < 768) initDrag();
    initKeys();
}
